...
namespace vehicleSeller
{
    public partial class engines : Form
    {
        ...
        {
            connection.Open();
            NpgsqlCommand command = new NpgsqlCommand("INSERT INTO engines(enginename) VALUES (@p1)", connection);
            command.Parameters.AddWithValue("@p1", enginename.Text);
        ...
        {
            connection.Open();
            NpgsqlCommand command = new NpgsqlCommand("UPDATE engines SET enginename=@p2 WHERE engineid=@p1", connection);
            command.Parameters.AddWithValue("@p1", dataGridView1.CurrentCell.Value);
            command.Parameters.AddWithValue("@p2", enginename.Text);
            command.ExecuteNonQuery();
            connection.Close();
        }
        ...
        {
            connection.Open();
            NpgsqlCommand command = new NpgsqlCommand("DELETE FROM engines WHERE engineid=@p1", connection);
            command.Parameters.AddWithValue("@p1", dataGridView1.CurrentCell.Value);
            command.ExecuteNonQuery();
            connection.Close();
        }
